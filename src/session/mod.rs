...
mod tests {
    ...
    fn populate(s: &mut Session, count: usize) {
        for i in 0..count {
            let role = if i % 2 == 0 {
                MessageRole::User
            } else {
            ...
            s.add_message(role, &format!("message body number {}", i));
        }
    }

    #[test]
    fn needs_compaction_triggers_above_threshold() {
        let mut s = Session::new("test", "test-model", 100);
        assert!(!s.needs_compaction(10));
        s.total_estimated_tokens = 95;
        assert!(s.needs_compaction(10));
        s.total_estimated_tokens = 89;
        assert!(!s.needs_compaction(10));
    }

    #[test]
    fn needs_compaction_with_zero_context_is_disabled() {
        let mut s = Session::new("test", "test-model", 0);
        s.total_estimated_tokens = u64::MAX / 2;
        assert!(!s.needs_compaction(10));
    }

    #[test]
    fn compress_replaces_summarized_messages_with_summary() {
        let mut s = Session::new("test", "test-model", 10_000);
        populate(&mut s, 8);
        let before_count = s.messages.len();
        let before_tokens = s.total_estimated_tokens;

        s.compress("SUMMARY of first 5".to_string(), 5, 200);

        assert_eq!(s.messages.len(), before_count - 5 + 1);
        assert_eq!(s.messages[0].role, MessageRole::System);
        assert!(s.messages[0].content.contains("SUMMARY of first 5"));
        assert_eq!(s.compactions.len(), 1);
        assert_eq!(s.compactions[0].summarized_count, 5);
        assert_eq!(s.compactions[0].first_kept_index, 1);
        assert_eq!(s.compactions[0].token_savings, 200);
        assert!(s.total_estimated_tokens < before_tokens);
    }

    #[test]
    fn compacted_context_returns_last_summary() {
        let mut s = Session::new("test", "test-model", 10_000);
        populate(&mut s, 4);
        assert!(s.compacted_context().0.is_none());
        s.compress("first summary".to_string(), 2, 50);
        let (summary, kept) = s.compacted_context();
        assert_eq!(summary, Some("first summary"));
        assert_eq!(kept, 1);
    }
}
